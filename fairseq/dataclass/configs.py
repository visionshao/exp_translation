...
class GenerationConfig(FairseqDataclass):
    ...
    # TODO: add importance features here
    imp_model_path: Any = field(
        default=None,
        metadata={"help":"if set, using importance sampling for generation"},
    )
    imp_model_databin: Any = field(
        default=None,
        metadata={"help":"the databin file path corresponding to the mono model path"}
    )
    sampling_size: int=field(
        default=0,
        metadata={"help":"the sampling vocab size for each beam of importance sampling, default 100"}
    )
    imp_gamma: float=field(
        default=0,
        metadata={"help":"the weight assigned to the importance model"}
    )
    imp_print: bool=field(
        default=False,
        metadata={"help":"if set true will output the importance of imp model"}
    )
