...
def override_module_args(args: Namespace) -> Tuple[List[str], List[str]]:
    ...
    if args is not None:
        ...
        if hasattr(args, 'mono_model'):
            overrides.append(f'generation.mono_model={args.mono_model}')
            overrides.append(f'generation.importance_sampling_size={args.importance_sampling_size}')
