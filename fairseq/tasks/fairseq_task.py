...
class FairseqTask(object):
    ...
    def build_generator(
        ...
        from fairseq.sequence_generator import (
            ...
            SequenceGeneratorWithImportance,
        )
        ...
        imp_model_path = getattr(args, "imp_model_path", False)
        imp_model_databin = getattr(args, "imp_model_databin", False)
        imp_gamma = getattr(args, "imp_gamma", False)
        sampling_size = getattr(args, "sampling_size", False)
        ...
        elif sampling_size!=0 and (imp_gamma or imp_model_path or imp_model_databin) is None:
            search_strategy=search.BeamSearchWithSamplingDecoding(
                self.target_dictionary, sampling_size,
            )
        elif sampling_size==0 and (imp_gamma or imp_model_path or imp_model_databin):
            assert imp_gamma and imp_model_path and imp_model_databin
            search_strategy=search.BeamSearchWithImportance(
                self.target_dictionary, imp_gamma
            )
        elif sampling_size!=0 and (imp_gamma or imp_model_path or imp_model_databin):
            assert imp_gamma and imp_model_path and imp_model_databin
            search_strategy = search.SamplingWithGammaSelection(
                self.target_dictionary, sampling_size, imp_gamma
            )
        else:
        ...
        if seq_gen_cls is None:
            ...
            elif (imp_model_databin and imp_model_path and imp_gamma) or sampling_size:
                seq_gen_cls = SequenceGeneratorWithImportance
                extra_gen_cls_kwargs["imp_print"] = args.imp_print
            else:
