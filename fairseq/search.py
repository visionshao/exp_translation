...
class BeamSearchWithSamplingDecoding(Search):
    """
    combine beam search with sampling decoding method
    for each beam generation, no longer take the top beam_size candidates for each beam, but random sampling sampling_size candidates,
    then select the all the beam_size x sampling_size candidates with top beam_size tokens as the result
    """

    def __init__(self, tgt_dict, sampling_size):
        super().__init__(tgt_dict)
        self.constraint_states = None
        self.sampling_size = sampling_size
        print("using beam search with importance sampling")
    ...
    def step(
            ...
            lprobs,
            scores: Optional[Tensor],
            imp_weights: Optional[Tensor] = None,
            prev_output_tokens: Optional[Tensor] = None,
            original_batch_idxs: Optional[Tensor] = None,
    ):
        bsz, beam_size, vocab_size = lprobs.size()
        # using beam search with importance sampling
        if step == 0:
            # at the first step all hypotheses are equally likely, so use
            # only the first beam
            lprobs = lprobs[:, ::beam_size, :]
            cum_lprobs = lprobs[:, ::beam_size, :].contiguous()

        else:
            # make probs contain cumulative scores for each hypothesis
            assert scores is not None
            cum_lprobs = lprobs + scores[:, :, step - 1].unsqueeze(-1)

        probs = lprobs.exp_()

        # random sampling sampling_size tokens for each beam
        if step == 0:
            sampled_indices = torch.multinomial(
                probs.view(bsz, -1),
                beam_size * self.sampling_size,
                replacement=True,
            ).view(bsz, beam_size, self.sampling_size)
        else:
            sampled_indices = torch.multinomial(
        ...
        # take the 2 x sampling_size x beam_size random sampling predictions
        if step == 0:
            cum_lprobs = cum_lprobs.expand(bsz, beam_size, -1)

        sampled_cum_lprob = torch.gather(cum_lprobs, dim=2, index=sampled_indices)

        top_vals, top_indices = torch.topk(sampled_cum_lprob.view(bsz, -1), k=min(
            beam_size * 2,
            sampled_cum_lprob.view(bsz, -1).size(1) - 1
        ))
        indices_buf = torch.gather(sampled_indices.view(bsz, -1), dim=1, index=top_indices)
        beams_buf = top_indices // self.sampling_size
        scores_buf = top_vals
...
class BeamSearchWithImportance(Search):
    """
    combine the importance weights into beam search decoding process,
    argmax gamma*imp_weights+(1-gamma)*lprobs
    """

    def __init__(self, tgt_dict, imp_gamma):
        super().__init__(tgt_dict)
        self.constraint_states = None
        self.imp_gamma = imp_gamma
    ...
    def step(
            ...
            prev_output_tokens: Optional[Tensor] = None,
            original_batch_idxs: Optional[Tensor] = None,
            bt_scores: Optional[Tensor] = None,
    ):
        bsz, beam_size, vocab_size = lprobs.size()
        ...
        if step == 0:
            ...
            lprobs = lprobs[:, ::beam_size, :].contiguous()
            bt_lprobs = lprobs
            imp_weights = imp_weights[:, ::beam_size, :].contiguous()
            gamma_lprobs = self.imp_gamma * imp_weights + (1 - self.imp_gamma) * lprobs
        else:
            bt_lprobs = lprobs + bt_scores[:, :, step - 1].unsqueeze(-1)
            gamma_lprobs = self.imp_gamma * imp_weights + (1 - self.imp_gamma) * lprobs + scores[:, :,
                                                                                          step - 1].unsqueeze(-1)

        top_prediction = torch.topk(
            gamma_lprobs.view(bsz, -1),
            k=min(
                # Take the best 2 x beam_size predictions. We'll choose the first
                # beam_size of these which don't predict eos to continue with.
                beam_size * 2,
        ...
        scores_buf = top_prediction[0]
        indices_buf = top_prediction[1]
        bt_scores_buf = torch.gather(bt_lprobs.view(bsz, -1), dim=1, index=indices_buf)
        # Project back into relative indices and beams
        beams_buf = indices_buf // vocab_size
        indices_buf = indices_buf.fmod(vocab_size)

        # At this point, beams_buf and indices_buf are single-dim and contain relative indices
        return scores_buf, indices_buf, beams_buf, bt_scores_buf


class BeamSearchWithImportanceSamplingDecoding(Search):
    """
    combine beam search with sampling decoding method
    for each beam generation, no longer take the top beam_size candidates for each beam, but random sampling sampling_size candidates,
    then select the all the beam_size x sampling_size candidates with top beam_size tokens as the result
    ...
    def __init__(self, tgt_dict, sampling_size, imp_gamma):
        super().__init__(tgt_dict)
        self.constraint_states = None
        self.sampling_size = sampling_size
        self.imp_gamma = imp_gamma
    ...
    def step(
        ...
        if step == 0:
            ...
            bt_lprobs = lprobs + bt_scores[:, :, step - 1].unsqueeze(-1)
            gamma_lprobs = self.imp_gamma * imp_weights + (1 - self.imp_gamma) * lprobs \
                           + scores[:, :, step - 1].unsqueeze(-1)

        probs = (self.imp_gamma * imp_weights + (1 - self.imp_gamma) * lprobs).exp_()
        ...
        # take the 2 x sampling_size x beam_size random sampling predictions
        if step == 0:
            gamma_lprobs = gamma_lprobs.expand(bsz, beam_size, -1)
            bt_lprobs = bt_lprobs.expand(bsz, beam_size, -1)

        sampled_gamma_lprobs = torch.gather(gamma_lprobs, dim=2, index=sampled_indices)
        sampled_bt_lprobs = torch.gather(bt_lprobs, dim=2, index=sampled_indices)

        top_vals, top_indices = torch.topk(sampled_gamma_lprobs.view(bsz, -1), k=min(
            beam_size * 2,
            sampled_gamma_lprobs.view(bsz, -1).size(1) - 1
        ))
        indices_buf = torch.gather(sampled_indices.view(bsz, -1), dim=1, index=top_indices)
        beams_buf = top_indices // self.sampling_size
        scores_buf = top_vals
        bt_lprobs_buf = torch.gather(sampled_bt_lprobs.view(bsz, -1), dim=1, index=top_indices)

        # At this point, beams_buf and indices_buf are single-dim and contain relative indices
        return scores_buf, indices_buf, beams_buf, bt_lprobs_buf


class SamplingWithGammaSelection(Search):
    """
    combine beam search with sampling decoding method
    for each beam generation, no longer take the top beam_size candidates for each beam, but random sampling sampling_size candidates,
    then select the all the beam_size x sampling_size candidates with top beam_size tokens as the result
    ...
    def __init__(self, tgt_dict, sampling_size, imp_gamma, len_norm=False):
        super().__init__(tgt_dict)
        self.constraint_states = None
        self.sampling_size = sampling_size
        self.imp_gamma = imp_gamma
    ...
    def step(
        ...
        if self.len_norm:
            gamma_lprobs = gamma_lprobs / step
        sampled_gamma_lprobs = torch.gather(gamma_lprobs, dim=2, index=sampled_indices)
        sampled_bt_lprobs = torch.gather(bt_lprobs, dim=2, index=sampled_indices)
