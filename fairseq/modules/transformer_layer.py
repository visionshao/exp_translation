...
class SegTransformerDecoderLayerBase(nn.Module):
    """Decoder layer block.
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def __init__(
        ...
        if no_encoder_attn:
            ...
            self.encoder_attn = self.build_segment_attention(self.embed_dim, cfg)
            self.encoder_attn_layer_norm = LayerNorm(self.embed_dim, export=cfg.export)
    ...
    def forward(
        ...
        context_attn_padding_mask: Optional[torch.Tensor] = None,
        self_attn_mask: Optional[torch.Tensor] = None,
        self_attn_padding_mask: Optional[torch.Tensor] = None,
        need_attn: bool = False,
        need_head_weights: bool = False,
        ...
        if self.encoder_attn is not None and encoder_out is not None:
            ...
            x, attn = self.encoder_attn(
                ...
                key=encoder_out,
                value=encoder_out,
                key_padding_mask=context_attn_padding_mask,
                incremental_state=incremental_state,
...
class SegTransformerEncoderLayerBase(nn.Module):
    """Encoder layer block.
